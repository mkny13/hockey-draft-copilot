...
**Security boundaries**: the Co-Pilot server is loopback-only by default (`http://localhost:3333`); the extension's `host_permissions` reach only the fantasy draft hosts (`fantasy.espn.com`, `*.fantasysports.yahoo.com`) and `localhost:3333`, not ESPN/Yahoo at large. No credentials or `.env` files are used anywhere. Untrusted draft-room text (player names, sync URLs) is escaped at render time before it reaches the DOM.
