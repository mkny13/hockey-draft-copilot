...
- **Board data:** `draft_data.json` / `public/draft_data.json` are a committed **sample** board (real names/ADP, synthetic FP/VORP) with the league settings (C2 F6 D6 UTIL1 G2 BN5, 8 teams). The author's real board lives in the gitignored `draft_data.local.json`, which `server.js` prefers (scripts and tests still read the sample); never rebuild VORP/FP from raw source data. Only ADP columns come from `scripts/ingest_espn_adp.js`. Roster limits are derived from `config.league.slots` in that file.
